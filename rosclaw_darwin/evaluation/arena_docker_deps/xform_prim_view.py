    ...
    def __init__(
        ...
        if validate_xform_ops:
            for prim in self._prims:
                # ROSClaw-Arena patch: Mesh/Scope prims from 6.0 kitchen assets may
                # lack standard translate/orient/scale ops. Standardize before validating;
                # if standardization cannot make it canonical, allow any xformable prim.
                try:
                    sim_utils.standardize_xform_ops(prim)
                except Exception:
                    pass
                if sim_utils.validate_standard_xform_ops(prim):
                    continue
                from pxr import UsdGeom

                if prim.IsA(UsdGeom.Xformable):
                    print(
                        f"[ROSCLAW PATCH] Allowing non-canonical xformable prim {prim.GetPath().pathString}"
