...
_DEFAULT_OUT_DIR = Path("data_v17/ablations/large_yaw_intervention")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Large-yaw targeted intervention ablation")
...
def main() -> None:
    ...
    for condition in args.conditions:
        path = _CONDITION_CONFIGS.get(condition, condition)
        condition_configs[condition] = _load_policy_config(path)

    rows: list[dict[str, Any]] = []
    for condition in args.conditions:
        policy_config = condition_configs[condition]
