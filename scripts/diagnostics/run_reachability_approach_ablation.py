#!/usr/bin/env python3
"""Reachability-aware approach ablation for workspace failure seeds.

Tests baseline and three reachability strategies on seeds that previously failed
with approach collision / workspace boundary.  Each condition is run serially to
avoid Docker/GPU contention.
"""
...
_DEFAULT_OUT_DIR = Path("data_v16/ablations/reachability_approach_ablation")

_CONDITION_OVERRIDES: dict[str, dict[str, Any]] = {
    "baseline": {"reachability_strategy": "direct_descend"},
    "high_pregrasp": {"reachability_strategy": "high_pregrasp"},
    "side_pregrasp": {"reachability_strategy": "side_pregrasp_positive_y"},
    "two_stage": {"reachability_strategy": "two_stage"},
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Reachability approach ablation for workspace failure seeds"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["7", "15", "28", "37", "48"],
        help="Seeds to evaluate (supports ranges like 7,15,28:30)",
    )
    parser.add_argument(
        "--conditions",
        type=str,
        default="baseline,high_pregrasp,side_pregrasp,two_stage",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Kill any running rosclaw-darwin:arena-base containers before each seed (default: True).",
    )
    parser.add_argument(
        "--no-cleanup",
        action="store_true",
...
def _aggregate(rows: list[dict[str, Any]], conditions: list[str]) -> dict[str, Any]:
    ...
    for condition in conditions:
        ...
        summary[condition] = {
            ...
            "mean_progress": (
                sum(progresses) / len(progresses) if progresses else None
            ),
        }
    return summary
